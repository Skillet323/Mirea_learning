...
// Удалили калибровочные переменные
const int noiseLevel = 62;  // Базовый уровень шума (тишина)
const int maxSignal = 400 - noiseLevel;  // Максимальный полезный сигнал
...
void handleModeAnimations() {
  ...
  switch(mode) {
    ...
    case 3:  // Музыка
      {
        static unsigned long prevMusicUpdate = 0;
        if(millis() - prevMusicUpdate < 10) break;
        prevMusicUpdate = millis();

        int micValue = analogRead(PIN_MIC);
        int signal = constrain(micValue - noiseLevel, 0, maxSignal);
        
        float logLevel = log10(signal + 1) / log10(maxSignal + 1);
        float dynamicLevel = pow(logLevel, 2.5);
        
        static float smoothedLevel = 0;
        smoothedLevel = 0.3 * dynamicLevel + 0.7 * smoothedLevel;
        
        int level = constrain(smoothedLevel * 255, 0, 255);
