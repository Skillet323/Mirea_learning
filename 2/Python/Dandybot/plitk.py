...
def load_tileset(filename):
    ...
    tileset = json.loads(filename.read_text())
    
    bin_file = Path(filename.parent / tileset["file"])
    
    if not bin_file.exists():
        raise FileNotFoundError(f"Бинарный файл не найден: {bin_file}")
    
    tileset["data"] = bin_file.read_bytes()
    return tileset
